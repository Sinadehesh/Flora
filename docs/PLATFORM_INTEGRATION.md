...
Both platforms open the same route: `floralock://challenge?source=<app name>`.
...
1. The shield shows a 🌸 icon, "Instagram is resting", "Name a flower to unlock",
   and the primary button **"Identify a flower"**.
2. The `ShieldAction` extension handles the tap by posting a **local notification**
   ("Tap to identify your flower") whose payload deep-links to
   `floralock://challenge?source=Instagram`, and responds `.close`/`.defer`.
3. The user taps the notification. FloraLock opens on the challenge screen.
4. On a correct answer, the app removes that app's token from the
   ...
   shield when the interval ends, even if FloraLock is killed.
...
- **Extensions share state via an App Group** (`group.com.floralock.app`):
...
- **Showing the quiz:** rather than drawing a `SYSTEM_ALERT_WINDOW` overlay with
  custom views, launch FloraLock's own activity with the deep link
...
- Budget: 3 photos per plant at 1000 px (mozjpeg, quality 74) averages about
  105 KB per photo, so today's 70 plants take 22 MB. At 400 plants that
  would be about 125 MB. At that size, bundle 1–2 photos per plant and
