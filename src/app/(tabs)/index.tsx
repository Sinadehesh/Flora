...
export default function Herbarium() {
  ...
  const iq = botanyIQ(deck, state.progress);
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.actions}>
        <Button
          label={due ? `Review ${due} due plant${due === 1 ? '' : 's'}` : 'Practice a flashcard'}
      ...
      {trouble.length === 0 ? (
        <Text style={{ color: c.textMuted, fontSize: 15 }}>
          Nothing yet. Plants you miss will show up here so you can study them.
        </Text>
      ) : (
        trouble.map((p) => {
